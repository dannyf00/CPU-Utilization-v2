...
void util_init(void) {
	...
	//Cortex-Mx implementation, with SysTick
	systick_init();							//if not already reset
}
...
uint32_t util_get(void) {
	...
	t0 = ticks();							//read the current tick / mapped to SysTick
	tmp = t0 - cnt;							//calculate ticks elapsed since last read
	cnt = t0;								//update _tick
	//option 1 - return the instantaneous measurement
